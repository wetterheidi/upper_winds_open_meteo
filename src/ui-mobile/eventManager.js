 ...
 * Außer "map" schließt ein erneuter Tap auf ein geöffnetes Panel dieses wieder (zurück zur Map).
 * @private
 */
...
function setupDeselectAllEnsembleButton() {
    ...
    deselectButton.addEventListener('click', () => {
        ...
        Utils.handleMessage(I18n.t('planner.deselect_all'));
    });
}
/**
    ...
    if (zoomMinInput && zoomMaxInput) {
        const updateZoomLevels = () => {
            ...
            if (String(minZoom) !== originalMinValue || String(maxZoom) !== originalMaxValue) {
                Utils.handleMessage(I18n.t('map.cache.zoom_autoadjust'));
            }
...
function setupAdsbEvents() {
    ...
    document.addEventListener('adsb:aircraftUpdated', (e) => {
        ...
        if (AppState.aircraftMarker && aircraft.lat && aircraft.lon) {
            ...
            if (aircraft.track) {
                // setRotationAngle() takes degrees directly (unlike web's setRotation() which takes radians)
