...
export function createSettingCheckbox(container, settingKey, labelText) {
    // NEU: Erstellen einer Container-div für die Reihe
    const row = document.createElement('div');
    row.className = 'setting-row';

    // Das Label für die Einstellung
    const label = document.createElement('label');
    label.textContent = labelText;
    label.htmlFor = settingKey; // Verknüpft das Label mit der Checkbox

    // Die Checkbox selbst
    const checkbox = document.createElement('input');
    checkbox.type = 'checkbox';
    checkbox.id = settingKey;
    checkbox.checked = Settings.state.userSettings[settingKey];
    checkbox.addEventListener('change', (event) => {
        Settings.state.userSettings[settingKey] = event.target.checked;
        Settings.save();
        // Optional: Ein Event auslösen, falls andere Teile der App reagieren müssen
        document.dispatchEvent(new CustomEvent('setting:changed', { detail: { key: settingKey, value: event.target.checked } }));
    });

    // NEU: Label und Checkbox zur Reihe hinzufügen
    row.appendChild(label);
    row.appendChild(checkbox);

    // NEU: Die komplette Reihe zum Akkordeon-Container hinzufügen
    container.appendChild(row);
}

export function createModelSelectionUI(containerId, settingKey) {
    const container = document.getElementById(containerId);
    if (!container) return;
    container.innerHTML = '';

    // --- NEU: Plattform-Erkennung ---
    // Wir prüfen, ob der Container innerhalb des mobilen Settings-Panels liegt.
    const isMobile = !!container.closest('#panel-settings');

    if (isMobile) {
        // --- Logik für die mobile App: Erzeuge ein <select> Dropdown ---
        
        // Erstelle eine Container-Reihe, die zu unserem neuen CSS-Layout passt
        const row = document.createElement('div');
        row.className = 'setting-row';

        const label = document.createElement('label');
        label.textContent = "Upper Winds Source"; // Statischer Text für das Label
        
        const select = document.createElement('select');
        select.className = 'model-select'; // Eigene Klasse zum Stylen

        Object.keys(WEATHER_MODELS).forEach(key => {
            const optionData = WEATHER_MODELS[key];
            const option = document.createElement('option');
            option.value = key;
            option.textContent = optionData.displayName;
            if (Settings.state.userSettings[settingKey] === key) {
        ...
        select.addEventListener('change', (event) => {
            Settings.state.userSettings[settingKey] = event.target.value;
            Settings.save();
            // Löst ein Event aus, damit die App die Wetterdaten neu laden kann
            document.dispatchEvent(new CustomEvent('setting:modelChanged')); 
        });
        ...
        // --- Bestehende Logik für die Web-App: Erzeuge Radio-Buttons ---
        Object.keys(WEATHER_MODELS).forEach(key => {
            const optionData = WEATHER_MODELS[key];
            const label = document.createElement('label');
            ...
            radio.type = 'radio';
            radio.name = settingKey;
            radio.value = key;
            radio.checked = Settings.state.userSettings[settingKey] === key;

            radio.addEventListener('change', () => {
                Settings.state.userSettings[settingKey] = key;
                Settings.save();
                document.dispatchEvent(new CustomEvent('setting:modelChanged'));
            });

            label.appendChild(radio);
            label.appendChild(document.createTextNode(` ${optionData.displayName}`));
