...
export function roundToTens(value) {
    return Math.round(value / 10) * 10;
}

export function convertTemperature(value, toUnit) {
...
export function convertWind(value, toUnit, fromUnit = 'km/h') {
    ...
    switch (fromUnit) {
        ...
        case 'bft':
            speedInKmH = beaufortToKnots(value) * 1.852;
            break;
        default:
            speedInKmH = value;
            break;
    ...
    switch (toUnit) {
        ...
        case 'bft':
            return knotsToBeaufort(speedInKmH / 1.852);
        default:
            return speedInKmH / 1.852;
    }
}
...
export function beaufortToKnots(bft) {
...
export function gaussianInterpolation(y1, y2, h1, h2, hp) {
...
export function interpolateWindAtAltitude(z, pressureLevels, heights, uComponents, vComponents) {
    ...
    const log_p_z = LIP(heights, log_pressureLevels, z);
    ...
    const u_z = LIP(log_pressureLevels, uComponents, Math.log(p_z));
    const v_z = LIP(log_pressureLevels, vComponents, Math.log(p_z));
...
export function LIP(xVector, yVector, xValue) {
    let reversed = false;
...
export function calculateMeanWind(heights, xComponents, yComponents, lowerLimit, upperLimit) {
    const dddff = new Array(4);
    let hLayer = [upperLimit];
    let xLayer = [Number(LIP(heights, xComponents, upperLimit))];
    let yLayer = [Number(LIP(heights, yComponents, upperLimit))];

    const xLower = Number(LIP(heights, xComponents, lowerLimit));
    const yLower = Number(LIP(heights, yComponents, lowerLimit));
    ...
    dddff[1] = windSpeed(xMean, yMean); // Speed
    dddff[0] = windDirection(xMean, yMean); // Direction

    return dddff;
}

// Cache for time zones and elevations
export function locationCache() {
    new Map();
}
...
export async function formatLocalTime(utcTimeStr, lat, lng) {
    if (!window.luxon) {
        console.warn('Luxon not available, falling back to UTC');
        return formatTime(utcTimeStr);
    }
    const { DateTime } = luxon;

    const { timezone, timezone_abbreviation } = await getLocationData(lat, lng);
 ...
 */
export function calculateWindAngle(trueCourse, windDirection) {
    let angle = normalizeAngle(windDirection - trueCourse);
 ...
 */
export function calculateWindComponents(windSpeed, windAngle) {
 ...
 */
export function calculateWCA(crosswind, trueAirspeed) {
 ...
 */
export function calculateGroundSpeed(trueAirspeed, headwind) {
    return trueAirspeed - headwind;
}

export function calculateCourseFromHeading(trueHeading, windDirection, windSpeed, trueAirspeed) {
    // Wind angle relative to heading
    const windAngle = calculateWindAngle(trueHeading, windDirection);
    const { crosswind, headwind } = calculateWindComponents(windSpeed, windAngle);
    ...
    const normalizedCourse = normalizeAngle(trueCourse);

    // Ground Speed
    const groundSpeed = Math.sqrt(gsU * gsU + gsV * gsV);

    // WCA (for reference)
    const wca = calculateWCA(crosswind, trueAirspeed) * (crosswind < 0 ? -1 : 1);
 ...
 */
export function calculateFlightParameters(trueCourse, windDirection, windSpeed, trueAirspeed) {
    const windAngle = calculateWindAngle(trueCourse, windDirection);
    const { crosswind, headwind } = calculateWindComponents(windSpeed, windAngle);
    const wca = calculateWCA(crosswind, trueAirspeed);
    const groundSpeed = calculateGroundSpeed(trueAirspeed, headwind);
...
export function handleError(message, log = true) {
...
export function decimalToMgrs(lat, lng) {
    try {
...
export function mgrsToDecimal(mgrsStr) {
    try {
...
export function convertCoords(lat, lng, format = 'Decimal') {
...
export function calculateTAS(ias, heightFt) {
...
export function calculateTASFromGroundSpeed(groundSpeed, windSpeed, windDirection, trueCourse, heightFt) {
    if (isNaN(groundSpeed) || isNaN(windSpeed) || isNaN(windDirection) || isNaN(trueCourse)) {
        return 'N/A';
    }
    const groundSpeedKts = convertWind(groundSpeed, 'kt', 'm/s');
    const windSpeedKts = convertWind(windSpeed, 'kt', 'm/s');
    const windAngle = calculateWindAngle(trueCourse, windDirection);
    const { crosswind, headwind } = calculateWindComponents(windSpeedKts, windAngle);
    const tasKts = groundSpeedKts + headwind;
    const tasAdjusted = calculateTAS(tasKts, heightFt);
